...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/cityscapes/cityscapes_bcanet101.yaml', help='config file')
    parser.add_argument('opts', help='see config/cityscapes/cityscapes_bcanet101.yaml for all options', default=None,
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == "bcanet":
        ...
        model = BCANet(num_classes=args.classes, BatchNorm=BatchNorm, layers=args.layers,
                             ...
                             criterion=criterion,pretrained=True)
    else:
    ...
    train_data = dataset.SemEdgeData(split='train', num_classes=args.classes, data_root=args.data_root,
    ...
    if args.evaluate:
        ...
        val_data = dataset.SemEdgeData(split='val', num_classes=args.classes, data_root=args.data_root,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train,bloss_train = train(train_loader, model, optimizer, epoch)
        if main_process():
            writer.add_scalar('loss_train', loss_train, epoch_log)
            writer.add_scalar('bloss_train', bloss_train, epoch_log)
        ...
        if args.evaluate and (epoch_log % args.val_freq == 0):
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, edge) in enumerate(train_loader):
        ...
        bloss_meter.update(edge_loss.item(),n)
        ...
        if main_process():
            ...
            writer.add_scalar('bloss_train_batch', bloss_meter.val, current_iter)
    ...
    if main_process():
        logger.info(
            'Train result at epoch [{}/{}]: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, args.epochs, mIoU,
                                                                                           mAcc, allAcc))
    return loss_meter.avg, mIoU, mAcc, allAcc,bloss_meter.avg
