...
class Compose(object):
    ...
    def __call__(self, image, label, bound):
        for t in self.segtransform:
            image, label, bound = t(image, label, bound)
...
class ToTensor(object):
    ...
    def __call__(self, image, label, bound):
        ...
        if not isinstance(bound, torch.LongTensor):
            bound = bound.long()
        return image, label, bound
...
class Resize(object):
    ...
    def __call__(self, image, label, bound):
        ...
        bound = cv2.resize(bound, self.size[::-1], interpolation=cv2.INTER_NEAREST)
        return image, label, bound
...
class RandScale(object):
    ...
    def __call__(self, image, label, bound):
        ...
        bound = cv2.resize(bound, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return image, label, bound
...
class Crop(object):
    ...
    def __call__(self, image, label, bound):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            bound = cv2.copyMakeBorder(bound, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half,
        ...
        bound = bound[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w]
        return image, label, bound
...
class RandRotate(object):
    ...
    def __call__(self, image, label, bound):
        if random.random() < self.p:
            ...
            bound = cv2.warpAffine(bound, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT,
                                   borderValue=0)
        return image, label, bound
